...
static int textfile_update(struct flb_lf *ctx)
{
    int ret;
    char errbuf[256];
    flb_sds_t contents;
    ...
    if (use_directory_pattern == FLB_TRUE) {
        ...
        ret = ne_utils_path_scan(ctx, ctx->path_textfile, dir_pattern, LF_SCAN_FILE, &list);
        if (ret != 0) {
            return -1;
        }
    }
        ...
        ret = ne_utils_path_scan(ctx, ctx->path_textfile, nop_pattern, LF_SCAN_FILE, &list);
        if (ret != 0) {
            return -1;
        }
    }
...
int lf_textfile_init(struct flb_lf *ctx)
{
...
int lf_textfile_update(struct flb_lf *ctx)
{
    textfile_update(ctx);
...
int lf_textfile_exit(struct flb_lf *ctx)
{
    return 0;
}
