...
/*-------------------------------------------------------------------------
 *
 *	  Utility functions for encoding conversions.
 *
...
format_byte_sequence_for_display(char *buf, size_t buf_size, const unsigned char *mbstr, int str_len)
{
    ...
    const int display_limit = 16; /* Max number of bytes to display */

    /* Determine how many bytes we will actually show in the log */
    int bytes_to_show = min(str_len, display_limit);

    for (int i = 0; i < bytes_to_show; i++)
    {
        /* Add a space before each byte except the first one */
        const char *prefix = (i == 0) ? "" : " ";

        /* Safely print the byte's hex value into the buffer */
        int written = snprintf(ptr, end - ptr, "%s0x%02x", prefix, mbstr[i]);

        /* If snprintf ran out of space or failed, stop processing */
    ...
    /* If we truncated the output, add an ellipsis to indicate more bytes exist */
...
{
    /* Define a buffer large enough for our display limit (16 * 5 chars) + extras */
    char buf[128];

    /* Determine the invalid character's length */
    int char_len = flb_encoding_mblen_or_incomplete(encoding, mbstr, len);

    /* If the length is unknown or exceeds remaining bytes, use remaining length */
    if (char_len == INT_MAX || char_len > len) {
        char_len = len;
    }

    /* Format the byte sequence for display */
                               ...
                               const char *mbstr, int len)
{
    ...
    /*
     * This path is not performance-critical, so we can be defensive.
     * Determine the character's length safely.
     */
...
#undef DO_RADIX_LOOKUP /* Clean up the macro to avoid namespace pollution */
                                  ...
                                  bool noError)
{
    ...
    /* Check if there's enough data for a potential second character */
    ...
    if (*len_ptr - first_char_len < next_char_len) {
        return 0; /* Not enough data for the second char */
    }
    ...
    if (next_char_len > 1) {
        ...
        if (cp) {
            /* Success! Update pointers and return consumed length. */
           ...
           int encoding, bool noError)
{
    ...
    while (len > 0) {
        ...
        /* 1. Try combined character map (if available) */
        if (cmap) {
            consumed = try_combined_conversion_from_utf8(&char_start_ptr, &len, &iso, char_code,
                                                             cmap, cmapsize, noError);
            if (consumed > 0) {
                utf = char_start_ptr;
                continue; /* Conversion successful, continue loop */
            }
            if (consumed < 0) {
